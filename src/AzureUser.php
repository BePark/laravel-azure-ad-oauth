...
class AzureUser
{
    ...
    public function refreshAccessToken()
    {
        ...
        $response = $guzzle->post('https://login.microsoftonline.com/common/oauth2/token', [
            'form_params' => [
                'client_id' => config('azure-oath.instances.azure-oauth.credentials.client_id'),
                'scope' => 'user.read',
                'refresh_token' => $this->get()->refreshToken,
                'redirect_uri' => config('azure-oath.instances.azure-oauth.credentials.redirect'),
                'grant_type' => 'refresh_token',
                'client_secret' => config('azure-oath.instances.azure-oauth.credentials.client_secret')
            ]
        ]);
